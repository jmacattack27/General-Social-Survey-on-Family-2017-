...
Figure \@ref(fig:religion) shows the distribution of responses when respondents were asked to rate the importance of religion in their lives. Here we see that the religion is still important to the majority of Canadians, with the most common answer being "Very important", with almost 7000 responses, followed by "Somewhat important" with just over 6000 responses. Conversely, the least common response was "Don't Know", with around 200 responses. About 13,000 respondents answered that religion was either "Very important" or "Somewhat important", with around 7000 respondents answering that religion was either "Not very important" or "Not at all important". This means that while religion still plays an important role in the majority of Canadian's lives, it is of little importance to a large minority of Canadians. 
...
# Plot for religion
religion_data %>%
  ggplot(aes(x = reorder(regilion_importance, n))) + # https://datavizpyr.com/re-ordering-bars-in-barplot-in-r/
  geom_bar() +
  theme_minimal() +
  theme(panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank(), 
        legend.position = "none") +
